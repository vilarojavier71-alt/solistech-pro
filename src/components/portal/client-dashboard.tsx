        ...
        return [
            {
                ...
                description: `Tu proyecto comenzó el ${new Date(sale.sale_date).toLocaleDateString('es-ES')}`,
                status: 'completed',
                date: sale.sale_date,
                icon: 'check',
            },
            {
                ...
                description: sale.payment_status === 'confirmed'
                    ? `Pago de €${sale.amount.toLocaleString('es-ES')} recibido correctamente`
            ...
            <div className="container max-w-4xl mx-auto p-4 py-8 space-y-8">
                ...
                <div className="space-y-2 text-center md:text-left">
                    ...
                    <p className="text-slate-600 text-lg">
                        Aquí tienes el estado actual de tu instalación solar.
                    </p>
                </div>
                    ...
                    <div className="md:col-span-2 space-y-6">
                        <Card className="border-0 shadow-xl shadow-slate-200/50 bg-white/70 backdrop-blur-sm rounded-3xl overflow-hidden">
                            <CardHeader className="bg-gradient-to-r from-slate-50 to-white border-b border-slate-100">
                                ...
                                <CardDescription>Tu camino hacia la energía solar</CardDescription>
                            </CardHeader>
                    ...
                    <div className="space-y-6">
                        ...
                        <Card className="border-0 shadow-lg shadow-slate-200/40 rounded-3xl bg-gradient-to-b from-sky-600 to-sky-700 text-white overflow-hidden relative">
                            ...
                            <CardHeader className="relative">
                                ...
                                <p className="text-sky-200 text-sm">Inversión Total</p>
                            </CardHeader>
                            <CardContent className="relative space-y-4 pt-0">
                                <div className="bg-white/10 rounded-xl p-3 backdrop-blur-md">
                                    ...
                                    <p className="font-medium">{sale.material || 'Configuración estándar'}</p>
                                </div>
                                <div className="bg-white/10 rounded-xl p-3 backdrop-blur-md">
                        ...
                        <Card className="border-0 shadow-lg shadow-slate-200/40 rounded-3xl bg-white">
                            <CardContent className="p-6 text-center space-y-4">
                                ...
                                <div>
                                    <h3 className="font-semibold text-slate-900">¿Tienes dudas?</h3>
                                    <p className="text-sm text-slate-500 mt-1">Nuestro equipo está aquí para ayudarte en cada paso.</p>
                                </div>
...
function getDocumentationDescription(status: string): string {
    switch (status) {
        case 'approved': return 'Documentación verificada y correcta. ¡Buen trabajo!'
        case 'rejected': return 'Hay un problema con algunos documentos. Por favor, revísalos.'
        case 'uploaded': return 'Hemos recibido tus archivos. Nuestro equipo los revisará pronto.'
        default: return 'Necesitamos tu confirmación de identidad y facturas para avanzar.'
    }
}

function getEngineeringDescription(status: string): string {
    switch (status) {
        case 'approved': return 'Ingeniería ha dado el visto bueno a tu proyecto técnico.'
        case 'rejected': return 'Hay detalles técnicos que ajustar antes de proceder.'
        case 'in_review': return 'Nuestro ingeniero está analizando la viabilidad y diseño.'
        default: return 'El equipo técnico preparará tu dossier personalizado.'
    }
}
...
function getProcessDescription(status: string): string {
    switch (status) {
        ...
        case 'presented': return 'Solicitudes presentadas a la administración pública.'
...
function getInstallationDescription(status: string, date: string | null): string {
    switch (status) {
        case 'completed': return '¡Instalación finalizada y funcionando! ☀️'
        case 'in_progress': return 'Nuestros técnicos están instalando tus paneles ahora.'
        case 'scheduled': return date ? `¡Fecha confirmada! Instalaremos el ${new Date(date).toLocaleDateString('es-ES')}` : 'Pronto te llamaremos para agendar la instalación.'
        default: return 'El último paso para disfrutar de tu propia energía.'
    }
}
