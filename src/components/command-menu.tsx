...
// import { Dialog, DialogContent } from "@/components/ui/dialog" // Usaremos el wrapper directo de cmdk si es necesario para estilos custom, o la implementaciÛn shadcn si existiera.
// Para esta implementaciÛn "Glass", usaremos estilos directos sobre CommandPrimitive para m·ximo control visual.
            ...
            <div className="w-full max-w-lg rounded-xl border border-white/10 bg-zinc-950/80 shadow-2xl backdrop-blur-xl animate-in zoom-in-95 overflow-hidden ring-1 ring-white/10">
                <CommandPrimitive className="flex h-full w-full flex-col overflow-hidden rounded-xl bg-transparent text-zinc-50">
                    ...
                    <CommandPrimitive.List className="max-h-[300px] overflow-y-auto overflow-x-hidden p-2">
                        ...
                        <CommandPrimitive.Group heading="NavegaciÛn R·pida" className="text-xs font-medium text-zinc-400 px-2 py-1.5">
                            ...
                            <CommandItem onSelect={() => runCommand(() => router.push('/dashboard/switching'))}>
                                ...
                                <span>Switching El√©ctrico</span>
                            </CommandItem>
                        </CommandPrimitive.Group>

                        <CommandPrimitive.Separator className="my-1 h-px bg-white/5" />

                        <CommandPrimitive.Group heading="ConfiguraciÛn" className="text-xs font-medium text-zinc-400 px-2 py-1.5">
...
function CommandItem({ children, onSelect }: { children: React.ReactNode, onSelect: () => void }) {
    return (
        ...
        >
            {children}
            <span className="ml-auto text-xs tracking-widest text-zinc-500 group-data-[selected=true]:text-zinc-400">‚Üµ</span>
        </CommandPrimitive.Item>
    )
}
